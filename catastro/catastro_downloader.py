...
class CatastroDownloader:
    ...
    def dibujar_contorno_en_imagen(
        ...
        try:
            ...
            if len(pixels) > 2:
                ...
                # Sistema de Siluetas Multicapa (Especificación Técnica)
                draw.line(pixels, fill=(255, 255, 255, 255), width=6)  # 1. Blanco exterior
                draw.line(pixels, fill=color + (255,), width=4)        # 2. Rojo principal
                draw.line(pixels, fill=(255, 255, 255, 255), width=2)  # 3. Blanco interior
                ...
                if (
                    ...
                    if PILLOW_AVAILABLE and response_catastro.status_code == 200:
                        try:
                            ...
                            # Simple alpha blend:
                            # 60% Ortofoto, 40% Plano (alpha=0.4 aplica 40% de la segunda imagen)
                            resultado = Image.blend(img_orto.convert("RGB"), img_catastro.convert("RGB"), alpha=0.4)
    ...
    def convertir_gml_a_kml(self, gml_path, kml_path=None):
        """Convierte un archivo GML a KML usando GeoPandas."""
        if not GEOTOOLS_AVAILABLE:
            print("  ⚠ GeoPandas no disponible, no se puede convertir a KML")
            return False
        ...
        try:
            ...
            if not gml_file.exists():
                print(f"  ✗ Archivo GML no encontrado: {gml_path}")
                return False
            
            # Si no se especifica ruta de salida, usar el mismo nombre con extensión .kml
            if kml_path is None:
            ...
            # Asegurar que está en WGS84 (EPSG:4326) que es requerido para KML
            if gdf.crs and str(gdf.crs) != "EPSG:4326":
                gdf = gdf.to_crs("EPSG:4326")
            
            # Extraer información útil para el KML
            # Intentar obtener la referencia catastral del nombre del archivo
            ref_catastral = gml_file.stem.replace('_parcela', '').replace('_edificio', '')
            
            # Agregar campos descriptivos al GeoDataFrame
            if 'Name' not in gdf.columns:
                tipo = 'Parcela' if 'parcela' in gml_file.name else 'Edificio'
                gdf['Name'] = f'{tipo} Catastral {ref_catastral}'
            
            if 'Description' not in gdf.columns:
                # Crear descripción HTML con información útil
                descripcion = f"""
                <![CDATA[
                <b>Referencia Catastral:</b> {ref_catastral}<br/>
                <b>Área:</b> {gdf.geometry.area.iloc[0]:.2f} m² (aprox.)<br/>
                <b>Enlaces:</b><br/>
                - <a href="https://www1.sedecatastro.gob.es/Cartografia/mapa.aspx?refcat={ref_catastral}">Ver en Catastro</a><br/>
                - <a href="https://www.google.com/maps/search/?api=1&query={gdf.geometry.centroid.y.iloc[0]},{gdf.geometry.centroid.x.iloc[0]}">Ver en Google Maps</a>
                ]]>
                """
                gdf['Description'] = descripcion
            ...
            print(f"  ✗ Error convirtiendo GML a KML: {e}")
            return False
    
    def generar_kmls_desde_gmls(self, ref):
        """Genera archivos KML a partir de todos los archivos GML de una referencia."""
        ref = self.limpiar_referencia(ref)
        
        # Buscar archivos GML en el directorio de salida
        archivos_gml = list(self.output_dir.glob(f"{ref}*.gml"))
        
        if not archivos_gml:
            print("  ⚠ No se encontraron archivos GML para convertir a KML")
            return False
        
        exito = False
        for gml_file in archivos_gml:
            if self.convertir_gml_a_kml(gml_file):
                exito = True
    ...
    def descargar_todo(self, referencia):
        ...
        # Generar archivos KML a partir de los GML descargados
        if parcela_gml_descargado or resultados.get('edificio_gml'):
            print("  🗺️ Generando archivos KML...")
            self.generar_kmls_desde_gmls(ref)
        
        # Generar Plano Perfecto (imagen con geometría sobre ortofoto)
        if parcela_gml_descargado and GEOTOOLS_AVAILABLE:
            print("  🎨 Generando Plano Perfecto...")
            try:
                ...
                if gml_path.exists():
                    ...
                    plano_perfecto_path = images_dir / f"{ref}_plano_perfecto.png"
                    self.generar_plano_perfecto(
                        gml_path=gml_path,
                        output_path=plano_perfecto_path,
                        ref=ref,
                        info_afecciones=None
                    )
                    resultados['plano_perfecto'] = True
            except Exception as e:
                print(f"  ⚠ Error generando Plano Perfecto: {e}")
                resultados['plano_perfecto'] = False
